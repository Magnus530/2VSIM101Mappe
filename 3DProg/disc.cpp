...
void Disc::draw()
{
    ...
    //glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mIBO);
    glDrawElements(GL_TRIANGLE_FAN, mIndices.size(), GL_UNSIGNED_INT, reinterpret_cast<const void*>(0));//mVertices.size());
}
...
//    mPosition.rotate(degrees, 0, );
    mMatrix = mRotation;

    mVelocity = QVector3D{0.1, 0 , 0};
